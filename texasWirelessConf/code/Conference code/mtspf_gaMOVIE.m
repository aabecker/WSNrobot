...
MOVIE_NAME = 'mTSP';
     configStruct.G.writerObj = VideoWriter(MOVIE_NAME,'MPEG-4');%http://www.mathworks.com/help/matlab/ref/videowriterclass.html
    set(configStruct.G.writerObj,'Quality',100)
    open(configStruct.G.writerObj);
    initialV = true;
...
% %initialize each salesman to a sector:
% theta = atan2(xy(2:end,1),xy(2:end,2));
% [~,thetaInd] = sort(theta);
% breaks = round(linspace(0,n,nSalesmen+1));
% numes = zeros(size(thetaInd));
% colors = zeros(size(thetaInd));
% for i = 1:numel(breaks)-1
%     numes(breaks(i)+1:breaks(i+1)) = i;
% end
% colors(thetaInd) = numes; 
% 
% %colors = ceil((theta+pi)/(2*pi)*nSalesmen);
% for k = 1:popSize
%     brN = 1;
%     for i =1:nSalesmen
%         indices = find(colors==i)+1;
%         r = randperm(numel(indices));
%         popRoute(k,brN:brN+numel(indices)-1) = indices(r);
%         brN = brN+numel(indices);
%         if i<nSalesmen
%             popBreak(k,i)=brN;
%         end
%     end
...
if showProg
    configStruct.G.fig = figure('Name','MTSPF_GA | Current Best Solution','Numbertitle','off');
...
for iter = 1:numIter
    ...
    for p = 1:popSize
        ...
        pRoute = popRoute(p,:);
        pBreak = popBreak(p,:);
        rngV = [[1 pBreak+1];[pBreak n]]';
        for s = 1:nSalesmen
            d = d + dmat(1,pRoute(rngV(s,1))); % Add Start Distance
            for k = rngV(s,1):rngV(s,2)-1
                d = d + dmat(pRoute(k),pRoute(k+1));
            end
            d = d + dmat(pRoute(rngV(s,2)),1); % Add End Distance
        end
        totalDist(p) = d;
    end
    ...
    if minDist < globalMin
        ...
        rngV = [[1 optBreak+1];[optBreak n]]';
        if showProg
            % Plot the Best Route
            for s = 1:nSalesmen
                rte = [1 optRoute(rngV(s,1):rngV(s,2)) 1];
                if dims > 2, plot3(hAx,xy(rte,1),xy(rte,2),xy(rte,3),'.-','Color',clr(s,:));
                else
                    configStruct.G.hPath(s) = plot(hAx,xy(rte,1),xy(rte,2),'-','Color',clr(s,:), 'linewidth',3);
                    hold(hAx,'on');
                configStruct.G.hWaypoints(s) = plot(xy(rte,1),xy(rte,2),'o','Color',clr(s,:),'markersize',12, 'linewidth',2);
                end
                hold(hAx,'on');
            end
            if dims > 2, hsink = plot3(hAx,xy(1,1),xy(1,2),xy(1,3),'o','Color',pclr);
            else hsink = plot(hAx,xy(1,1),xy(1,2),'*','Color',pclr,'markersize',12, 'linewidth',2); end
            title(hAx,sprintf('mTSP, Distance = %1.1f, Iteration = %d',minDist,iter));
            hold(hAx,'off');
                 xlabel 'X-axis (m)'
    ylabel 'Y-axis (m)'
            legend([configStruct.G.hWaypoints(1),configStruct.G.hPath(1),hsink],{'path waypoints','path of UV','sink'},'location','eastOutside')
    axis equal
    axis tight
...
close(configStruct.G.writerObj);
