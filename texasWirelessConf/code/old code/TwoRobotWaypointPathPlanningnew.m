function TwoRobotWaypointPathPlanningnew()
...
%    waypoints2=10*[x2',y2'];
  n1=10;
   ...
   waypoints1 = [cos(theta1)+5,sin(theta1)+5];
   n2 = 10;
   theta2 = linspace(0,2*pi*(1-1/n2),n2)';
    waypoints2 = [cos(theta2)-5,sin(theta2)-5];
WAYPOINTS=[0 0;waypoints1;0 0;0 0;waypoints2;0 0];
%disp(WAYPOINTS);
f = figure(1);
...
cellsz = mY(2) - mY(1);
sensors1=[-10,-10;10,10;-8,-10;-10,-8;8,10;10,8];%Intresting points  0,0;-2,2;2,2;-2,-2;2,-2;-6,6;-8,8;-4,8;-8,4;-4,4;6,-6;8,-8;4,-8;8,-4;4,-4;
...
%                  colormap copper;
        ...
        hold on
        hWaypoints1 = plot(WAYPOINTS(1:size(waypoints1)+2,1),WAYPOINTS(1:size(waypoints1)+2,2),'go');
        hWaypoints2 = plot(WAYPOINTS(size(waypoints1)+3:end,1),WAYPOINTS(size(waypoints1)+3:end,2),'ro');
        axis equal
        axis tight
        hPath1 = line([WAYPOINTS(1:size(waypoints1)+2,1);WAYPOINTS(1,1)],[WAYPOINTS(1:size(waypoints1)+2,2);WAYPOINTS(1,2)],'color','m');
        hPath2 = line([WAYPOINTS(size(waypoints1)+2:end,1);WAYPOINTS(size(waypoints1)+2,1)],[WAYPOINTS(size(waypoints1)+2:end,2);WAYPOINTS(size(waypoints1)+2,2)],'color','c');
