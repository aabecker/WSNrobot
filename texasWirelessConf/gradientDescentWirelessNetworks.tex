...
Let $\mathbf{p}_i^r$ be the position of the $i^{th}$, $i\in(1\ldots n(r))$ waypoint of the $r^{th}$ UV. Servicing includes recharging the nodes and collecting a part of the data that the nodes are about to transmit to the sink there by reducing the power expenditure in the sensor nodes. The algorithm forms a locally-optimal path to visit the sensor nodes in the WSN.  At each step, we compute the Voronoi partition $\mathbf{V}_i^r$ defined by the waypoints, with one partition assigned to each waypoint.
 ...
 We require a function  $\phi(\mathbf{q})$(interesting function) that designates how many sensor nodes can be recharged from the position $\mathbf{q}$. It tells the positions of the sensors and indicates the $\mathbf{r_{footprint}}$ of each sensor. This information is necessary for minimizing the distance traveled by the UV and when incorporated into the map it provides information for charging multiple sensor nodes from a single location. For the simulation experiments we use a binary interesting function. A simple function is used but the int resting function can be designed by taking parameters like max height at which the robot starts charging, rate of charging etc and can be made better to mimic the real time presence. Here the function calculates the distance between each sensor node and every other point on the map, if this distance is less that the constant $\mathbf{P}$ then that point would be interesting. As mentioned this function is binary and can be improved.
 \begin{align}\label{eq:interesting-function}
 \mathbf{\phi(q)}  = {(\mathbf{X}_s-\mathbf{X}_q)}^2 + {(\mathbf{Y}_s-\mathbf{Y}_q)}^2 < P 
 \end{align}
 Here $(\mathbf{X}_s,\mathbf{Y}_s)$ correspond to the location(X and Y co-ordinate) of the sensor and $(\mathbf{X}_q,\mathbf{Y}_q)$ correspond to the location of every point on the map $\mathbf{q}\subset\mathbf{Q}$.
\begin{figure}\centering 
\newcommand{\figheight}{1.4in}
\subfigure[\label{subfig:nodePositions}]
  {\includegraphics[width=.45\columnwidth]{phi_1}}
 \subfigure[\label{subfig:PhiwithTransCost}]
  {\includegraphics[width=.45\columnwidth]{phi_2}}
   \vspace*{-.1in}
 \caption{The interesting function provides the range from which the UV will be able to service the sensor node. \label{fig:Phi_interesting}}
 \vspace*{-.1in}
\end{figure}
%commands the usefulness of a location on the map for servicing the WSN (${\mathbf{q}\in{Q}|\phi(\mathbf{q})>0}$) .
 Definition of cost function for each path is given by:
...
\sum\limits_{i=1}^{n(r)}\frac{\mathbf{W}_n}{2}\|p_i^r-p_{i+1}^r\|^2
\end{align}
...
The same local optimization techniques presented in this paper can iteratively adapt the paths of UVs.  A schematic of our the adaptive control law is shown in Fig.~\ref{fig:dynamicVsStatic}.  
