...
\begin{figure} \centering
 ...
 \caption{ As the recharging footprint or data-transfer footprint $r_{footprint}$ grows, more sensors can be recharged simultaneously. The plot above shows $K_{footprint}(r_{footprint})$, calculated by \eqref{eq:Kfootprint}.
 \label{fig:footprint}}
\end{figure}
...
Let $\mathbf{p}_i^r$ be the position of the $i^{th}$, $i\in(1\ldots n(r))$ waypoint of the $r^{th}$ UV. Servicing includes recharging the nodes and collecting a part of the data that the nodes are about to transmit to the sink, thereby reducing the power expenditure in the sensor nodes. The algorithm forms a locally-optimal path to visit the sensor nodes in the WSN.  At each step, we compute the Voronoi partition $\mathbf{V}_i^r$ defined by the waypoints, with one partition assigned to each waypoint.
 ...
 \phi(\mathbf{q})  = \sum_{s = 1}^k {(x_s-\mathbf{q}_x)}^2 + {(y_s-\mathbf{q}_y)}^2 < r_{footprint}
 \end{align}
...
\subfigure[\label{subfig:nodePositions}]
 ...
 \subfigure[\label{subfig:PhiwithTransCost}]
  {\includegraphics[width=.45\columnwidth]{phi_2}}
   \vspace*{-.1in}
 \caption{Function \eqref{eq:interesting-function}  determines the range from which a UV can service a sensor node. \label{fig:Phi_interesting}}
 \vspace*{-.1in}
\end{figure}
...
\mathbf{H} = \sum\limits_{r=1}^N\sum\limits_{i=1}^{n(r)} \int_{V_i^r}\frac{W_s}{2}\|\mathbf{q}-\mathbf{p}_i^r\|^2&\phi(\mathbf{q}) d\mathbf{q}&+\nonumber\\
\sum\limits_{r=1}^N\sum\limits_{i=1}^{n(r)}\frac{W_n}{2}\|\mathbf{p}_i^r-\mathbf{p}_{i+1}^r\|^2
\end{align}
