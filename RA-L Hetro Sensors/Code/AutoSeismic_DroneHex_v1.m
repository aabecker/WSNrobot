function [] = AutoSeismic_DroneHex_v1(x,y,T,hex)
...
if nargin <1
    ...
    darts = 25; %total number of Darts
    drone_cap = 4;
end
%constants
...
for k = 1:S.drones  % 0- unassigned 1- assigned
    if  S.Qstate(k) == 0 %only assign if unassigned
        if S.Qdeploying(k) == 0 % need to retrieve darts -- so find the closest dart
            ...
            for m = 1:numel(S.surveyPtsX)
                if S.surveyPtsState(m) == 0 %no measurement & unassigned
                    dist = sqrt(sum((S.Qpos(k,:) - [S.surveyPtsX(m),S.surveyPtsY(m)]).^2))+...
                        sqrt(sum((S.Tpos - [S.surveyPtsX(m),S.surveyPtsY(m)]).^2));
                    if dist<minDist
                        minInd = m;
                        minDist = dist;
...
xlabel 'X-dist (m)';
ylabel 'Y-dist (m)';
% title 'Seismic Surveying using Heterogenous Sensors';
%draw truck
